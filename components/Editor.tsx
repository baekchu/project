...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "./Form/Tabs";
...
export default function Editor({ data, onChange, documentURL }: Props) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "editor w-full min-h-[70vh] border-input border-none px-3 py-1 transition-colors focus-visible:outline-none",
      },
    },
  });
  ...
  const getImageURL = async (file: File) => {
    const storageRef = ref(
      storage,
      `images/postImages/${documentURL}/${file.name}`
    );
  ...
  function onImageURSubmit(values: z.infer<typeof urlForm>) {
